...
class PROPEN_MLP(nn.Module):
    ...
    def __init__(self, x_dim, args):
        ...
        #                 dropout=args.dropout, activation=args.activation, slope=args.slope, device=args.device)
        self.pG_X = nn.Linear(in_features=x_dim, out_features=1, bias=True)
        # heads for G=E
        ...
        #                 dropout=args.dropout, activation=args.activation, slope=args.slope, device=args.device)
        self.pT_XE = nn.Linear(in_features=x_dim, out_features=1, bias=True)
        # heads for G=O
        ...
        #                 dropout=args.dropout, activation=args.activation, slope=args.slope, device=args.device)
        self.pT_XO = nn.Linear(in_features=x_dim, out_features=1, bias=True)
    ...
    def forward(self, X):
        # X_base = self.base(X)
        prob_G = self.sigmoid(self.pG_X(X))

        # G=E group
        # hidden_E = self.base_E(X_base)
        prob_TE = self.sigmoid(self.pT_XE(X))
        # prob_TE = self.sigmoid(self.pT_XE(X_base))

        # G=O group
        # hidden_O = self.base_O(X_base)
        prob_TO = self.sigmoid(self.pT_XO(X))
...
def NEWS(args):
    ...
    if args.save:
        import csv
        save_dic = {}
        save_dic['method'] = 'two_stage_pro_logit'
