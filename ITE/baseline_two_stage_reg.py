...
class REG_MLP(nn.Module):
    ...
    def construct_YMR(self, X, T, S, Y, G, epsilon=1e-6):
        ...
        Y_MR = muYO1 - muYO0 + muSE1 - muSE0 + muSO0 - muSO1
...
def TWO_STAGE_REG(X, S, Y, T, G,
    ...
    mlp = REG_MLP(x_dim=X.shape[1], args=args)
    ...
    if cross_fitting is True:
        ...
        mlp2 = REG_MLP(x_dim=X.shape[1], args=args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr_2', type=float, default=1e-3, help='Initial learning rate.')
    parser.add_argument('--weight_decay_2', type=float, default=5e-4, help='Weight decay.')
