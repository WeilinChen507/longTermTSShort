...
class REGX_MLP(nn.Module):
    ...
    def construct_YMR(self, X, T, S, Y, G, epsilon=1e-6):
        ...
        Y_MR = (1 - G) * ((1 - T) * (muYO1 - Y + S - muSO1) +
                          T * (Y - muYO0 - S + muSO0) + muSE1 - muSE0) + \
               G * (muYO1 - muYO0 + muSO0 - muSO1 +
                    (1 - T) * (muSE1 - S) + T * (S - muSE0))
...
def TWO_STAGE_REG(X, S, Y, T, G,
    ...
    mlp = REGX_MLP(x_dim=X.shape[1], args=args)
    ...
    if cross_fitting is True:
        ...
        mlp2 = REGX_MLP(x_dim=X.shape[1], args=args)
...
def IHDP(args):
    ...
    for i, (train, valid, test) in enumerate(lti.get_train_valid_test()):
        ...
        ite_predictor = TWO_STAGE_REG(x_train, np.squeeze(s_train), np.squeeze(y_train),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_layers_2', type=int, default=2, help='epoch of training discriminator')
    parser.add_argument('--dropout_2', type=float, default=0.2, help='Dropout rate (1 - keep probability).')
