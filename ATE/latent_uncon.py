...
def imputationApproach(X, S, Y, T, G, regressionType='linear'):
    ...
    pT_XE = e_x_estimator(XE, TE)
    propensity = pT_XE.predict_proba(XE)[:, 1][:, None]  # p(T=1|X=x,G=E)
    ...
    # print(np.sum(TE * k1.predict(np.concatenate((XE, SE), 1)) / propensity ) / np.sum(np.ones_like(TE)) -
    #       np.sum((1 - TE) * k0.predict(np.concatenate((XE, SE), 1))/(1-propensity))  / np.sum(np.ones_like(TE)))

    # print(np.sum(TE * k1.predict(np.concatenate((XE, SE), 1)) / propensity * ratio ) / np.sum(ratio) -
    #       np.sum((1 - TE) * k0.predict(np.concatenate((XE, SE), 1))/(1-propensity)  * ratio) / np.sum(ratio))
...
def weightingApproach(X, S, Y, T, G, kernelType='gaussian'):
    ...
    tau = np.sum(YO * TO * weight_lambda / ex_proba) / np.sum(TO * weight_lambda / ex_proba) - \
          np.sum(YO * (1 - TO) * weight_lambda / (1 - ex_proba)) / np.sum((1 - TO) * weight_lambda / (1 - ex_proba))
...
def con_cdf(kde_join, kde_con, S,T,X, sample_gap=1e-1):
    ...
    :return:
    '''
    # problematic method that should be rewritten
...
def dgp():
    ...
    TE = np.random.binomial(n=1, size=Esize, p=(1 / (1 + np.exp(-np.mean(3*XE, axis=1)))))
    ...
    YE_treated = 4 + np.mean(XE, axis=1) + np.mean(XE, axis=1) + SE_treated + noiseE
