...
class MLP(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim, n_layers,
                 dropout=None, activation='none', slope=.1, device='cpu'):
        super().__init__()
        ...
        for act in self.activation:
            if act == 'lrelu':
                ...
                self._act_f.append(lambda x: F.elu(x))
            elif act == 'xtanh':
...
class TrainedMLP:
    ...
    def __init__(self, input_dim, output_dim, hidden_dim, n_layers, activation='none', slope=.1, device='cuda'):
        ...
        self.mlp=MLP(input_dim, output_dim, hidden_dim, n_layers, activation=activation, slope=slope, device=device)
        self.mlp.to(device)
        self.device = device


    def fit(self, X,Y, optimizer='Adam', epochs=100, lr=1e-3, batch_size=64):
        X,Y = torch.tensor(X).to(self.device), torch.tensor(Y).to(self.device)
        lossfuc = nn.MSELoss(reduction='none')
        if optimizer == 'Adam':
            optimizer = optim.Adam(self.mlp.parameters(), lr=lr)

        # dataset = TensorDataset(X,Y[:, None])
...
class TrainedMLPLinear(nn.Module):

    def __init__(self, input_dim, output_dim=1, regressionType='linear', device='cuda', bias = True):
        super().__init__()
        torch.set_default_tensor_type(torch.DoubleTensor)
        self.linear=nn.Linear(in_features=input_dim,out_features=output_dim, bias=bias)
        if regressionType is not 'linear':
            self.linear = MLP(input_dim, output_dim, hidden_dim=32, n_layers=3, activation='lrelu', slope=.1,
                           device=device)
    ...
    def fit(self, X,Y, factor, optimizer='Adam', epochs=100, lr=1e-3, batch_size=200):
        X,Y = torch.tensor(X).to(self.device), torch.tensor(Y).to(self.device)
        factor = torch.tensor(factor).to(self.device)
        lossfuc = nn.MSELoss(reduction='none')
        if optimizer == 'Adam':
        ...
        # dataset = TensorDataset(X,Y[:, None])
        # print(X.shape, Y.shape, factor.shape)
        dataset = TensorDataset(X,Y,factor)
        train_loader = DataLoader(dataset, batch_size=batch_size, shuffle=True)

        for epoch in range(epochs):
            for x, y,factor_batch in train_loader:
                optimizer.zero_grad()
                outputs = self.forward(x, factor_batch)
                # print(outputs.shape)
                # print(y.shape)
                assert outputs.shape == y.shape
                loss = lossfuc(outputs, y)
    ...
    def predict(self, X):
        X = torch.tensor(X).to(self.device)
        # return self.forward(X, factor)  self.linear(X)
        return self.linear(X).cpu().detach().numpy().reshape(-1)

    def coef_(self):
        return str(self.linear.weight.data.cpu().detach().numpy())\
            + '  '\
            + str(self.linear.bias.data.cpu().detach().numpy())

class TrainedLogistic_G(nn.Module):

    def __init__(self, device='cuda'):
        torch.set_default_tensor_type(torch.DoubleTensor)
        super().__init__()
        # =\frac{1}{ \frac{3+2\exp(-2x)}{3\exp(-2x)+2 }a +1 } where a= (1-p(E))/p(E)
        self.w1, self.w2= nn.Parameter(torch.tensor(torch.randn(1))), nn.Parameter(torch.tensor(torch.randn(1)))
        self.w3, self.w4= nn.Parameter(torch.tensor(torch.randn(1))), nn.Parameter(torch.tensor(torch.randn(1)))
        self.w5, self.w6= nn.Parameter(torch.tensor(torch.randn(1))), nn.Parameter(torch.tensor(torch.randn(1)))
    ...
    def forward(self, x):
        # prob = (self.w1 + torch.exp( self.lin1(x) )) / (self.w2 + torch.exp( self.lin2(x) ) + 1e-8)
        # print((1/(1 + prob)).shape, prob.shape)
        prob = (self.w1 + self.w2 * torch.exp(self.w5*x)) / (self.w3 + self.w4 * torch.exp(self.w5*x) + 1e-6)
        # prob = 1/(1 + prob)

        # print(prob.shape)
        # print(prob)
        prob = torch.clamp_max(prob, 1)
        prob = torch.clamp_min(prob, 0)
        return prob

    def fit(self, X,Y, optimizer='Adam', epochs=400, lr=1e-3, batch_size=32):
        X,Y = torch.tensor(X).to(self.device), torch.tensor(Y).to(self.device)
        lossfuc = nn.BCELoss()
        if optimizer == 'Adam':
            optimizer = optim.Adam(self.parameters(), lr=lr)#, weight_decay=5e-4)
            # optimizer = optim.SGD(self.parameters(), lr=lr, weight_decay=1e-5)
        ...
        for epoch in range(epochs):
            for x, y in train_loader:
                ...
                assert outputs.shape == y[:,None].shape
                loss = lossfuc(input=outputs, target=y[:, None])
                loss.backward()
                optimizer.step()
    ...
    def predict(self, X):
        x = torch.tensor(X).to(self.device)
        y = self.forward(x)
        y = y.cpu().detach().numpy()    # posibility of Y=1
        return np.squeeze(y)
...
class TrainedLogisticMLP:
    ...
    def fit(self, X,Y, optimizer='Adam', epochs=200, lr=1e-3, batch_size=64):
        X,Y = torch.tensor(X).to(self.device), torch.tensor(Y).to(self.device)
        lossfuc = nn.BCELoss()
        if optimizer == 'Adam':
            optimizer = optim.Adam(self.mlp.parameters(), lr=lr)
